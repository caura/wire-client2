...
z.main.App = class App {
  ...
  _setup_repositories() {
    ...
    repositories.event_tracker       = new z.tracking.EventTrackingRepository(repositories.user, repositories.conversation);
  ...
  _setup_services() {
    ...
    services.client                  = new z.client.ClientService(this.auth.client, service.storage);
    services.notification            = new z.event.NotificationService(this.auth.client, service.storage);
  ...
  on_internet_connection_gained() {
    ...
    this.auth.client.execute_on_connectivity()
      .then(() => {
        ...
        this.repository.event.reconnect_web_socket(z.event.WebSocketService.prototype.CHANGE_TRIGGER.ONLINE);
      });
  }
  ...
  on_internet_connection_lost() {
    ...
    this.repository.event.disconnect_web_socket(z.event.WebSocketService.prototype.CHANGE_TRIGGER.OFFLINE);
  ...
  _subscribe_to_beforeunload() {
    $(window).on('beforeunload', () => {
      ...
      this.repository.event.disconnect_web_socket(z.event.WebSocketService.prototype.CHANGE_TRIGGER.PAGE_NAVIGATION);
  ...
  logout(cause, clear_data = false, session_expired = false) {
    const _logout = () => {
      ...
      this.repository.event.disconnect_web_socket(z.event.WebSocketService.prototype.CHANGE_TRIGGER.LOGOUT);
  ...
  disable_debugging() {
    ...
    this.repository.properties.save_preference_enable_debugging(false);
  }
  ...
  enable_debugging() {
    ...
    this.repository.properties.save_preference_enable_debugging(true);
  }
...
$(function() {
  if ($('#wire-main-app').length !== 0) {
    return wire.app = new z.main.App(wire.auth);
  }
});
