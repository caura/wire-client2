...
z.event.EventRepository = class EventRepository {
  ...
  _handle_event(event) {
    ...
    if (z.event.EventTypeHandling.IGNORE.includes(event_type)) {
      this.logger.info(`Event ignored: '${event_type}'`, {event_object: event, event_json: JSON.stringify(event)});
  ...
  _validate_call_event_lifetime(event) {
    ...
    if (corrected_timestamp > (event_timestamp + EVENT_CONFIG.E_CALL_EVENT_LIFETIME)) {
      this.logger.info(`Ignored outdated '${event.type}' event in conversation '${event.conversation}' - Event: '${event_timestamp}', Local: '${corrected_timestamp}'`, {event_object: event, event_json: JSON.stringify(event)});
