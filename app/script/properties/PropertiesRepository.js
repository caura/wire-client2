...
z.properties.PropertiesRepository = class PropertiesRepository {
  ...
  init(self_user_et) {
    ...
    .then((keys) => {
      ...
      if (keys.includes(z.config.PROPERTIES_KEY)) {
        return this.properties_service.get_properties_by_key(z.config.PROPERTIES_KEY)
        .then((properties) => {
  ...
  save_preference(properties_type, updated_preference) {
    ...
    if (updated_preference !== current_preference) {
      ...
      this.properties_service.put_properties_by_key(z.config.PROPERTIES_KEY, this.properties)
      .then(() => {
